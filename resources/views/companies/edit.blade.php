        ...
        <div class=" col-md-9 col-sm-9 pull-left">
            ...
            <!-- Three columns of text below the carousel -->
                <div class="row pt-5 col-md-12 col-lg-12 col-sm-12" style=" margin: 10px;">
                    <div class="container">
                        <form action="{{ route('companies.update',[$company->id])}}" class="form-control" method="POST">
                            {{ csrf_field() }}
                            <input type="hidden" name="_method" value="put">

                            <div class="form-group">
                                <label for="company-name">Name</label>
                                <input type="text" name="name" id="company-name" class="form-control" placeholder="Enter name" value="{{ $company->name}}" required>
                            </div>

                            <div class="form-group">
                                <label for="company-content">Description</label>
                                <textarea name="description" id="company-content" class="form-control autosize-target text-left" placeholder="Enter description" rows="5" style="resize: verticle;">{{ $company->description}}</textarea>
                            </div>
